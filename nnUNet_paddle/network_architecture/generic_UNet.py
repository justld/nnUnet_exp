...
from nnUNet_paddle.network_architecture.initialization import InitWeights_He
from nnUNet_paddle.network_architecture.neural_network import SegmentationNetwork
...
class ConvDropoutNormNonlin(nn.Layer):
    def __init__(self, input_channels, output_channels, conv_op=nn.Conv2D, conv_kwargs=None,
                norm_op=nn.BatchNorm2D, norm_op_kwargs=None, dropout_op=nn.Dropout2D, 
                dropout_op_kwargs=None, nonlin=nn.LeakyReLU, nonlin_kwargs=None):
        super().__init__()
        ...
        if norm_op_kwargs is not None:
            norm_op_kwargs = {'epsilon': 1e-5, 'momentum': 0.9}
...
class StackedConvLayers(nn.Layer):
    def __init__(self, input_feature_channels, output_feature_channels, num_convs,
        ...
        if norm_op_kwargs is None:
            norm_op_kwargs = {'eps': 1e-5, 'momentum': 0.9}
        if conv_kwargs is None:
            conv_kwargs = {'kernel_size': 3, 'stride': 1, 'padding': 1, 'dilation': 1, 'bias_attr': True}

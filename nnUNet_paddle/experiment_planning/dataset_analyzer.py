...
from nnUNet_paddle.utils import load_pickle, load_json, save_pickle
from nnUNet_paddle.preprocessing.cropping import get_patient_identifiers_from_cropped_files
...
class DatasetAnalyzer:
    ...
    Args:
        folder_with_cropped_data (str): cropped data folder for nnunet. 
        overwrite (bool): Whether overwrite precomputed data. Default: True.
        num_processes (int): Number of threads. Default: 8.
    """
    def __init__(self, folder_with_cropped_data, overwrite=True, num_processes=8):
        ...
        assert os.path.isfile(os.path.join(self.folder_with_cropped_data, 'dataset.json')), "dataset.json needs to be in folder_with_cropped_data."
        self.props_per_case_file = os.path.join(self.folder_with_cropped_data, "props_per_case.pkl")
        self.intensityproperties_file = os.path.join(self.folder_with_cropped_data, "intensityproperties.pkl")
    ...
    def _get_unique_labels(self, patient_identifier):
        seg = np.load(os.path.join(self.folder_with_cropped_data, patient_identifier) + ".npz")['data'][-1]
    ...
    def _load_seg_analyze_classes(self, patient_identifier, all_classes):
        ...
        """
        seg = np.load(os.path.join(self.folder_with_cropped_data, patient_identifier) + ".npz")['data'][-1]
        pkl = load_pickle(os.path.join(self.folder_with_cropped_data, patient_identifier) + ".pkl")
    ...
    def get_classes(self):
        datasetjson = load_json(os.path.join(self.folder_with_cropped_data, "dataset.json"))
    ...
    def analyse_segmentations(self):
        ...
        if self.overwrite or not os.path.isfile(self.props_per_case_file):
    ...
    def _get_voxels_in_foreground(self, patient_identifier, modality_id):
        all_data = np.load(os.path.join(self.folder_with_cropped_data, patient_identifier) + ".npz")['data']
    ...
    def analyze_dataset(self, collect_intensityproperties=True):
        ...
        save_pickle(dataset_properties, os.path.join(self.folder_with_cropped_data, "dataset_properties.pkl"))
